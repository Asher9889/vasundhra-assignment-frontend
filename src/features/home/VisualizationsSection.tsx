...
export function VisualizationsSection({ title, description, datasets }: VisualizationsSectionProps) {
    ...
    <section aria-label={title}>
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {datasets.map(({ dataset, detailHref }) => (
          <VisualizationCard key={dataset.id} dataset={dataset} detailHref={detailHref} />
        ))}
      </div>
    </section>
  )
